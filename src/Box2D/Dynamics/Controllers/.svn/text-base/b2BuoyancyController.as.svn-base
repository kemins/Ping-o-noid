...
public class b2BuoyancyController extends b2Controller
{
	...
	public override function Step(step:b2TimeStep):void{
		...
		for (var i:b2ControllerEdge = m_bodyList; i; i = i.nextBody)
		{
			...
			for (var fixture:b2Fixture = body.GetFixtureList(); fixture; fixture = fixture.GetNext())
			{
				...
				if (useDensity) {
					//: Figure out what to do now density is gone
					shapeDensity = 1;
				}else{
					shapeDensity = 1;
				}
			...
			//Angular drag
			//: Something that makes more physical sense?
			body.ApplyTorque( -body.GetInertia() / body.GetMass() * area * body.GetAngularVelocity() * angularDrag)	
			
			//apply a random downward impulse once and a while
			//if(Utilities.GetRandomNumberInARange(100, 1) < 8)
			//body.ApplyImpulse(new b2Vec2(0, Utilities.GetRandomNumberInARange(10, 1) / 800), new b2Vec2(0, 0));
		}
	}
	...
	//adds a downward tap every 2 seconds
